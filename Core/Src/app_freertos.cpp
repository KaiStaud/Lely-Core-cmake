...
auto write_object(uint16_t index, uint8_t subindex, uint32_t value) -> bool;
...
//  HAL_TIM_PWM_Start(&htim8, TIM_CHANNEL_2);
  ...
  /* Infinite loop */
  for (;;) {
  ctrl_word = read_object(0x6040, 0);
  ...
  write_object(0x6041, 0x00, statusword);
    /*
...
auto read_object(uint16_t index,uint8_t subindex) -> uint32_t
{
  ...
  if (entry != NULL) {
      ...
      if (OD_getSub(entry, subindex, &io, false) == ODR_OK) {
          ODR_t ret = OD_get_u16(entry, subindex, &value, false);
...
auto write_object(uint16_t index, uint8_t subindex, uint32_t value) -> bool
{
  ...
  if (entry != NULL) {
      OD_IO_t io;
      uint16_t u16_value = static_cast<uint16_t>(value);

      if (OD_getSub(entry, subindex, &io, true) == ODR_OK) {
          return OD_set_u16(entry, subindex, u16_value, true) == ODR_OK;
      }
  }
  return false;
}
